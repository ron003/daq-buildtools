...
```sh
dbt-create.sh <daq release name>
```
...
This can be easily done by editing the `.dunedaq_area` file generated by `dbt-create.sh` and adding the new entries to `dune_products_dirs`  and `dune_products` as needed. 
