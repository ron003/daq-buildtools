...
This can be easily done by editing the `.dunedaq_area` file generated by `dbt-init.sh` and adding the new entries to `dune_products_dirs`  and `dune_products` as needed. 
